...
//debug                    printf("%i: %i,%i -> %i,%i\n",a,j,i,jj,ii);
                    ...
                    {
                        printf("Crash! Cart %i hit cart %i at %i,%i (tick %i). Remaining carts: %i\n",a,track2[jj][ii],jj,ii,tick,carts-2);
                        track2[jj][ii]=0; //remove cart crashed into
                        carts-=2; //decrease cart count
        ...
        if(carts==1)
        {
            for(int i=0;i<150;i++)
            {
                for(int j=0; j<150;j++)
                {
                    if(track2[j][i]>0)
                    {
                        printf("Part 2: Last cart standing: cart %i at %i,%i\n",track2[j][i],j,i);
                        return 0;
                    }
                }
            }
